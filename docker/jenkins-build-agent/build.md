...
the Dockerfile — edit them to bump. Bun is pinned as `ARG BUN_VERSION` and
helmfile as `ARG HELMFILE_VERSION` (binaries fetched from their GitHub
releases). JDK 17/21 pull the latest Temurin GA patch when not already cached;
JDK 25 comes from the `jenkins/inbound-agent:trixie-jdk25` base tag.
